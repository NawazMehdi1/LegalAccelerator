...
type RelatedServiceProps = ComponentProps & {
  fields: {
    data: {
      item: { relatedServices?: { targetItems?: Array<targetItemsType> } };
    };
  };
};

const RelatedServices = (props: RelatedServiceProps): JSX.Element => {
  const { t } = useI18n();
  if (!props?.fields?.data?.item?.relatedServices?.targetItems?.length) {
    return <></>;
  }
  return (
    <div className="related-topic" data-testid="related-topics">
      <div className="max-w-[1200px] 2xl:max-w-[1440px] mx-[1.375rem] md:mx-[2.125rem] xl:m-auto py-[3.5rem]">
        ...
        <div className="flex flex-col md:flex-wrap print:flex print:flex-row gap-y-[1.25rem] md:flex-row">
          {props?.fields?.data?.item?.relatedServices?.targetItems?.map(
            (item: targetItemsType, index: number) => (
              <a
                key={index}
                href={props?.fields?.data?.item?.relatedServices?.targetItems?.[index]?.url?.path}
