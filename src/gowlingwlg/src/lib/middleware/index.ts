...
export default async function middleware(
  ...
  //geolocation redirect here
  // const country = req.geo?.country ?? '';

  //check if language is selected, language selection in LanguageSelector
  // const setLanguage = req.cookies.get('language')?.value;
  // const setLocation = req.cookies.get('location')?.value;

  // const AVAILABLE_REGIONS = ['GB', 'CA', 'CN', 'FR', 'DE', 'AE', 'US'];

  // const newUrl =
  //   req.nextUrl.origin + '/en-' + country + (req.nextUrl.pathname ? req.nextUrl.pathname : '');

  // if (AVAILABLE_REGIONS.includes(country) && !(setLanguage || setLocation)) {
