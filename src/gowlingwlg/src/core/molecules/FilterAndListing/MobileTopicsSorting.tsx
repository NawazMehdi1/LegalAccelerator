...
const TopicsSorting = ({ setFilter, activeItems, items }: TopicPropsType) => {
  const { t } = useI18n();
  ...
  const activeTopicsArry = [...activeItems];
      ...
      >
        {items &&
          items.map((item, index) => {
            const isActive = activeItems.includes(item.text);
            return (
              <button
                onClick={() => {
                  isActive
